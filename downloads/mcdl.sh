...
require_env() {
  if [[ -z "${!1}" ]]; then
    ...
    echo "Нажмите Stop, создайте новый секрет(secret) названный $1 и назначте ему $2 ,"
    echo "затем снова запустите repl в режиме установки после ngrok_token,"
    echo "и в режиме запуска сервера после ngrok_region."
    exit
  fi
}
require_executable() {
